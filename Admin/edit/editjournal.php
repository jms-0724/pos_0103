...
if (isset($jsondecoded['add_journal_number'])) {
    ...
    try {
        ...
        $uid = $_SESSION['userid'];
        $upd_journal_date = $datetoday;
        $upd_journal_date2 = $jsondecoded['upd_journal_date'];
        $upd_journal_number = $jsondecoded['upd_journal_number'];
        $upd_journal_description = $jsondecoded['upd_journal_description'];
        $upd_journal_category = $jsondecoded['upd_journal_category'];
        $fiscal_id = $_SESSION['fiscal_id'];

        $sql1 = $conn->prepare("UPDATE tbl_journal_entry SET journal_date = :journal_date, description = :description, category_id =:category_id, uid = :uid, fiscal_id = :fiscal_id WHERE journal_voucher_no = :journal_voucher_no");
        $sql1->bindParam(':journal_voucher_no', $upd_journal_number);
        $sql1->bindParam(':journal_date', $upd_journal_date2);
        $sql1->bindParam(':description', $upd_journal_description);
        ...
        if ($sql1->execute()) {
            ...
            foreach ($jsondecoded['journal_array'] as $row) {
                ...
                $sql2 = $conn->prepare("UPDATE tbl_journal_items SET account_code = :account_code, journal_amount = :journal_amount, journal_adjustment = :journal_placement WHERE journal_voucher_id = :journal_voucher_id");
